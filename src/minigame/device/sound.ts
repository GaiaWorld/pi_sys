...
import { ResTab, Res } from '../../pi_sys/modules/util/res_mgr';
...
class Sound {
    ...
    public play(volume?: number, delay?: number, offset?: number): void {
        ...
        if ((!this.res) && this.playTime !== undefined) {
            return;
        }

        // this.res.link.play();

        let s: HTMLAudioElement = this.res.link;
        // if (isWXMiniGame) {
        //     s = this.src = context;
        ...
        // s = this.src = SoundResTab.context;
        // s.buffer = s.src = this.res.link;
        // // TODO: 现在还没有实现start方法的delay和offset参数，后续实现
        // s.start = s.play;
        ...
        delay = delay ? (<HTMLAudioElement>this.res.link).currentTime + delay : 0;
        offset = offset || this.pauseTime;

        // s.start(delay, offset);
        s.play();

        this.playTime = (<HTMLAudioElement>this.res.link).currentTime + delay - offset;
        this.pauseTime = 0;
    ...
    public pause(): void {
        ...
        this.src.stop();
        this.pauseTime = (<HTMLAudioElement>this.res.link).currentTime - this.playTime;
        if (this.pauseTime < 0) {
            this.pauseTime = 0;
        }
        this.playTime = 0;
    ...
    public destroy(): void {
        ...
        globalSoundResTab.delete(this.res);
        // (<any>this.res.link).destroy && (<any>this.res.link).destroy();
...
export class SoundResTab extends ResTab {
    ...
    constructor() {
        super();
        if (globalSoundResTab) {
            console.warn(`SoundResTab 已被初始化过一次。`);
        } else {
            globalSoundResTab = this;
        }
    }
    ...
    public play(src: string, delay?: number, repeat?: number, repeatDelay?: number): void {
        ...
        cfg.mgr = this;
        cfg.startTime = 0 ; //SoundResTab.context.currentTime;
        cfg.delay = delay || 0;
        cfg.repeat = repeat || 0;
...
const play = (cfg: SoundCfg, res: Res) => {
    ...
    const d = cfg.delay + cfg.startTime - res.link.currentTime;
