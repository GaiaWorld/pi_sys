 ...
 * @param e down 坐标
 */
export const downCall = (e: { x: number, y: number }) => {
    let rule = ruleList[downRecord];
    if (rule.x[0] < e.x && e.x < rule.x[1]) {
        if (rule.y[0] < e.y && e.y < rule.y[1]) {
            downRecord++;
        } else {
            downRecord = 0;
        }
