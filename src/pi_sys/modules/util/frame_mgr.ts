...
export const create = () => {
    ...
    /**
     * @description 添加一个永久调用函数和参数（参数必须为数组）, 放在队列后
     * @example
     */
    frame.setPermanent = (func: Function, args?: any[]) => {
        func && permanent.unshift(func, args);
    };
    /**
     * @description 添加一个永久调用函数和参数（参数必须为数组）, 放在队列前
     * @example
     */
    frame.setPermanentBefore = (func: Function, args?: any[]) => {
