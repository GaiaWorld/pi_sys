...
export class Tree<K, V> {
	...
	keys(key?: K): IterableIterator<K> {
		return new ItemIter(getKey, this.root, this.cmp, key)
	...
	values(key?: K): IterableIterator<V> {
		return new ItemIter(getValue, this.root, this.cmp, key)
	...
	items(key?: K): IterableIterator<{ key: K, value: V }> {
		return new ItemIter(getItem, this.root, this.cmp, key)
...
// // 从指定的键开始迭代
// const iterKey = function* K(node: Node<K, V>, cmp: (a: K, b: K) => number, key: K, get: (node: Node<K, V>) => T) {
...
// 因为小游戏不支持yield, 所以重新实现
class ItemIter<K, V, T> implements IterableIterator<T> {

	private get: (node: Node<K, V>) => T;
	private arr: Node<K, V>[] = [];

	constructor(get: (node: Node<K, V>) => T, node: Node<K, V>, cmp: (a: K, b: K) => number, key?: K) {
		this.get = get;
		if (key) {
			while (node != null) {
				const r = cmp(key, node.key);
